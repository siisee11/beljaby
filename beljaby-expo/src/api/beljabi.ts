...
export async function changeJoin() {
    const response = await axios.post(`/new/join/change`)
    return response.data
}

export async function getChampionForSummoner() {
    const response = await axios.get(`/get/summoner/champion`)
    return response.data
}
